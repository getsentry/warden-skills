...
6. **SAML assertion with signed-assertion gate and IdP-metadata-bound certificate.** Enterprise SAML where the relying party verifies the XML signature against a certificate fetched from admin-configured metadata satisfies the session-check equivalent for the email attribute. Do not flag unless the signing check is missing or the certificate source is attacker-controlled.
