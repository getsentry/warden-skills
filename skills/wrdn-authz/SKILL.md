...
- **Pure authentication lifecycle bugs** (login, password reset, session fixation, MFA, account recovery) unless the bug directly feeds a protected authorization decision. **IdP/SSO identity claim trust** (trusting IdP email to resolve or link users, orphaned identity reattachment, `email_verified` signal corruption) is a separate concern unless the forged identity directly enables a role or permission escalation.
