  ...
  (it) => {
    it(
      ...
      async ({ run, behavior, harness }) => {
        ...
        const result = await run("Please audit .github/workflows/test.yml for security vulnerabilities.");
    ...
    it(
      "no-bare-pull-request__artifact-handoff-exception",
      { timeout: 120_000 },
      async ({ run, behavior, harness }) => {
        behavior("no-bare-pull-request");
        await harness.useFixture("no-bare-pull-request__artifact-handoff-exception");
